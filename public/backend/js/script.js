...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(document).ready(function () {
    ...
    /* var isFullScreen = sessionStorage.getItem('isFullScreen');
    
    if (isFullScreen === 'true') {
        // You can apply any logic here if you want to ensure full-screen
        console.log("The screen was in full-screen mode at the start of the session.");
    } else {
        console.log("The screen was not in full-screen mode at the start of the session.");
    } */
  });
  ...
  /*=====================
    ...
    /* if(window.location.pathname.includes("layout-dark.html")){
      ...
      const updateIframeStyles = () => {
        $('.richtexteditor').each(function () {
            ...
            const $editorIframe = $richTextEditor.find('iframe');
    
            if ($editorIframe.length) {
                let innerDoc;
                try {
                    innerDoc = $editorIframe[0].contentDocument;
                } catch (error) {
                    console.log("Unable to access iframe contentDocument due to CORS restrictions.");
                    return;
                }
    
                const $body = $('body');
                const modeType = localStorage.getItem("mode");
                const isDarkMode = (modeType === "dark-only" || $body.hasClass('dark-only'));
    
                if (innerDoc) {
                    const $rteToggleBorder = $(innerDoc).find('.rte-toggleborder');
                    if ($rteToggleBorder.length) {
                        $rteToggleBorder.css('color', isDarkMode ? '#A1A1A1' : 'black');
                    }
                }
            }
        });
    ...
    $(document).on('load', '.richtexteditor iframe', function () {
        updateIframeStyles();
    });
        ...
        $(".dark-mode").on("click", function () {
            ...
            // if (!window.location.pathname.includes("layout-dark.html")) {
                if (!bodyModeDark) {
                    ...
                    $("body").removeClass("light");
                    /*  */
                    localStorage.setItem("primary", "#3a60f8");
                    localStorage.setItem("secondary", "#c6c6c6");
                } else {
                    $(".dark-mode").removeClass("active");
                    localStorage.setItem("mode", "light");
                    $("body").removeClass("dark-only");
                    $("body").addClass("light");
                    /*  */
                    localStorage.setItem("primary", "#04196c");
                    localStorage.setItem("secondary", "#808080");
                }
                const  darkMode = $(this).hasClass('active');                
                $.ajax({
                  url: '/admin/dark-mode',
                  method: 'POST',
                  contentType: 'application/json',
                  data: JSON.stringify({ mode: darkMode }),
                  success: () => {
